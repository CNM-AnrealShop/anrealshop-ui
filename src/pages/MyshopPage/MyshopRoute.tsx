...
const ShopAdminHeader = lazy(
  () => import("../../components/header/ShopAdminHeader")
);
const MessagePage = lazy(
  () => import("../../components/Myshop/Message/MessagePage")
);
...
const CreateProduct = lazy(
  () => import("../../components/Myshop/Product/Create/CreateProduct")
);
const ProductForm = lazy(
  () => import("../../components/Myshop/Product/Create/CreateProduct")
);
const ProductPage = lazy(
  () => import("../../components/Myshop/Product/ProductPage")
);
const Setting = lazy(
  () => import("../../components/Myshop/Setting/SettingPage")
);
const OrderShippingPage = lazy(
  () => import("../../components/Myshop/Order/OrderShippingPage")
);
const OrderPrintPage = lazy(
  () => import("../../components/Myshop/Order/OrderPrintPage")
);

const MyshopPage = () => {
  const { isAuthenticated, shop, user, status } = useAppSelector(
    (state) => state.auth
  );
      ...
      if (
        ...
        if (!user?.hasShop) {
          ...
          showErrorNotification(
            "Thông báo",
            "Bạn cần đăng ký cửa hàng trước khi truy cập."
          );
        }
      }
    }, 2000);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 bg-gray-50">
        <Routes>
          <Route
            index
            element={<Navigate to={APP_ROUTES.MYSHOP.DASHBOARD} replace />}
          />
          <Route
            path={APP_ROUTES.MYSHOP.DASHBOARD}
