...
const OrderHistory = () => {
  ...
  }, [activeStatus, fetchOrders, itemsPerPage, updateParams, searchType]);
  ...
  const handleCancelOrder = useCallback(
    (orderItemId: string, reason: string) => {
      rejectShopOrder(orderItemId, reason).then(() => {
        loadOrders();
      });
    },
  ...
  const handleBuyAgain = (productIds: string[]) => {
    ...
    showSuccessNotification(
      "Hệ thống",
      "Chức năng mua lại đang được phát triển"
    );
  };
